...
router.get('/requests', requireHeadAdmin, async (req, res, next) => {
  try {
...
router.post('/requests/:id/approve', requireHeadAdmin, async (req, res, next) => {
  const { id } = req.params;
  if (!isUuid(id)) return res.status(400).json({ error: 'Invalid request ID' });
  ...
  try {
    ...
    const reqResult = await client.query('SELECT * FROM admin_requests WHERE id = $1', [id]);
...
router.post('/requests/:id/reject', requireHeadAdmin, async (req, res, next) => {
  const { id } = req.params;
  if (!isUuid(id)) return res.status(400).json({ error: 'Invalid request ID' });
...
router.get('/stats', async (req, res, next) => {
  try {
    const [allReports, reports, users, pending, approved, rejected, revenue, branches, adminRequests] = await Promise.all([
      ...
      pool.query("SELECT COUNT(DISTINCT branch) FROM monthly_reports"),
...
router.get('/audit-logs', async (req, res, next) => {
  ...
  const limit = Math.min(parseIntSafe(req.query.limit, 50), 100);
